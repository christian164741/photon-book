...
\subsubsection*{Experimental Bounds:}
\phantomsection
...
$$
\index{Photon mass}
This number $10^{-54}~\text{kg}$ is not a law of nature nor a theoretical prediction , but an upper bound inferred from many experiments and observations – under the hypothesis that the photon might have a mass. No such tiny deviations have been observed.
...
\begin{tcolorbox}[hypobox, title={What if the Photon Had a Mass?}]
	...
	\begin{itemize}
		...
		\item The energy–frequency relation would not be $E=hf$, but would follow the relativistic energy–momentum relation:
		$$
	...
	\end{itemize}
Since none of these effects are observed, the photon is, with overwhelming experimental evidence, truly massless.
\end{tcolorbox}
\vspace{1em}
\newpage
\noindent
